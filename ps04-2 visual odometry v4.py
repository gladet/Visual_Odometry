...
frame_start = 700 #0 #80 #4 #100 #80 #100 #4 #80 #0 #80 #100 #4 #80 #100 #4 #0
frame_end = 900 #999 #120 #6 #102 #120 #102 #6 #120 #999 #120 #102 #6 #120 #102 #6 #10
...
def epipolar_match(l_keypoints, r_keypoints, l_descript, r_descript, cam, y_thresh):
    ...
    for lidx,lk in enumerate(l_keypoints):
        ...
        for ridx,rk in enumerate(r_keypoints):
            ...
            disparity = lk[1] - rk[1]
            if ((rk[0] >= lk[0] - y_thresh and rk[0] <= lk[0] + y_thresh) 
                and (disparity >= disparity_threshold and disparity <= max_disparity)):
...
def determine_motion(image_x, image_y, pts_3D, cam):
    ...
    def apply_motion(motion):
        ...
        #original version of tz / tx
        #tz = d*c
        #tx = d*s
        #my version of tz / tx
        #tz = d
        #tx = 0
        '''RT = np.array([[ R[0, 0], R[0, 1], R[0, 2], tx],
                       [ R[1, 0], R[1, 1], R[1, 2], ty], #ty
                       [ R[2, 0], R[2, 1], R[2, 2], tz],
                       [ 0, 0, 0,  1]]) #[R/T] -> transformation matrix'''
        '''RT = np.array([[ cy, 0, sy, tx],
                       [ sx * sy, cx, -sx * cy, ty], #ty
                       [ -cx * sy, sx, cx * cy, tz],
                       [ 0, 0, 0,  1]]) #[R/T]'''
